...
class LinkedList:
    ...
    def __contains__(self, element):
        ...
        if self._size == 0 : False
        else:
            # starts the process by passing down the head and element
            return self._contains(self._head, element)

    def _contains(self, node, element):
        # if the element of the node == argument, True
        if node._element == element:
            return True
        # if no next node we havent found the element
        elif node._next == None:
            return False

        # if conditions are not meet, iterate again
    ...
    def insert_at_tail(self, element):
        ...
        # if LL is empty push eqlement to the head
        if self._size == 0: self.insert_at_head(element)
        # else push them to the tail
        else: self._add_at_tail(self._head, element)

    def _add_at_tail(self, node, element):
        if node._next != None:
                        
            self._add_at_tail(node._next, element)
        else:
            # if the LL is empty we should push this to the head
            # if there is a next node then add it to the tail
            self._size = self._size + 1
